...
function updateStats() {
    const total = labCategories.length;
...
function getFilteredCategories() {
    return labCategories.filter(cat => {
...
function validateCategoryForm(name, code, isEditMode = false, currentId = null) {
    ...
    if (!name || name.trim() === '') {
        ...
        const duplicate = labCategories.some(cat => {
...
function saveCategory(e) {
    ...
    if (isEdit) {
        const index = labCategories.findIndex(c => c.id === currentId);
        if (index !== -1) {
            labCategories[index] = { ...labCategories[index], name, code, description };
            saveCategories();
            refreshUI();
        ...
        labCategories.push({ id: newId, name, code, description });
        saveCategories();
        refreshUI();
...
function handleConfirmDelete() {
    ...
    const category = labCategories.find(c => c.id === deleteTargetId);
    labCategories = labCategories.filter(c => c.id !== deleteTargetId);
    saveCategories();
    
    // Also delete tests under this category
...
function initLabCategoriesModule() {
...
document.addEventListener('DOMContentLoaded', function() {
    const checkInterval = setInterval(() => {
        ...
        if (sidebar) {
            ...
            setTimeout(initLabCategoriesModule, 100);
        }
    }, 50);
    
    setTimeout(() => {
        clearInterval(checkInterval);
        initLabCategoriesModule();
    }, 3000);
});